from core.advbase import *
from module.bleed import Bleed
from slot.a import *
from slot.d import *

def module():
    return Addis

class Addis(Adv):
    comment = 's2 c2 s1 c5fsf c4fs s1; hold s2s1 until bleed under 3'

    a3 = ('bk',0.20)
    conf = {}
    conf['slots.a'] = Resounding_Rendition()+Breakfast_at_Valerios()
    conf['acl'] = """
        `dragon(c3-s-end), s4.check()
        `s3, not buff(s3)
        `s4, not buff(s2)
        `s2, s1.charged>=s1.sp-260 and bleed.get() < 3
        `s1, s2.charged<s2.sp and bleed.get() < 3
        `fsf, xf=5 and buff(s2)
        """
    conf['coabs.base'] = ['Akasha','Dragonyule_Xainfried','Lin_You']
    conf['share'] = ['Curran']
    conf['afflict_res.poison'] = 0

    def getbleedpunisher(self):
        if self.bleed._static['stacks'] > 0:
            return 0.08
        return 0

    def prerun(self):
        random.seed()
        # self.s2buff = Selfbuff('s2_shapshifts1',1, 10,'ss','ss')
        # self.s2str = Selfbuff('s2_str',0.25,10)
        self.bleedpunisher = Modifier('bleed','att','killer',0.08)
        self.bleedpunisher.get = self.getbleedpunisher
        self.bleed = Bleed('g_bleed',0).reset()
        # self.crit_mod = self.rand_crit_mod

    def s1_proc(self, e):
        if e.group == 'enhanced' and random.random() < 0.8:
            Bleed(e.name, 1.32).on()
        # if self.s2buff.get():
        #     self.s2buff.buff_end_timer.add(2.5)
        #     self.s2str.buff_end_timer.add(2.5)
        #     if random.random() < 0.8:
        #         Bleed(e.name, 1.32).on()
        # else:
        #     self.afflics.poison(e.name,100,0.53)


    # def s2_proc(self, e):
    #     self.s2buff.on()
    #     self.s2str.on()


if __name__ == '__main__':
    from core.simulate import test_with_argv
    test_with_argv(None, *sys.argv)